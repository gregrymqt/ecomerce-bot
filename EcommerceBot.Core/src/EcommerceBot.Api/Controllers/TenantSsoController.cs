...
public class TenantSsoController : BaseApiController
{
    ...
    /// </summary>
    [HttpPost("mappings")]
    public async Task<IActionResult> CreateMapping([FromBody] CreateTenantSsoMappingRequest request, CancellationToken cancellationToken = default)
    {
        var tenantId = CurrentTenantId;
        if (tenantId == Guid.Empty)
            return BadRequestProblem("Tenant não identificado no contexto da requisição.");
    ...
    public async Task<IActionResult> UpdateMapping([FromRoute] Guid id, [FromBody] UpdateTenantSsoMappingRequest request, CancellationToken cancellationToken = default)
    {
        var tenantId = CurrentTenantId;
        if (tenantId == Guid.Empty)
            return BadRequestProblem("Tenant não identificado no contexto da requisição.");
    ...
    public async Task<IActionResult> DeleteMapping([FromRoute] Guid id, CancellationToken cancellationToken = default)
    {
        var tenantId = CurrentTenantId;
        if (tenantId == Guid.Empty)
            return BadRequestProblem("Tenant não identificado no contexto da requisição.");
