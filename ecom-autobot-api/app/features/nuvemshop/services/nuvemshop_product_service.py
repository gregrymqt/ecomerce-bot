...
class NuvemshopProductService:
    """
    Serviço de Lógica de Negócio para Gestão de Catálogo e Produtos da Nuvemshop.
    ...
    async def enqueue_bulk_sync(
        ...
        logger.info(
            f"🚀 [NuvemshopProductService] Job '{job_id}' enfileirado com sucesso: {len(skus)} SKUs para o tenant '{self.tenant_id}'."
        )
    ...
    async def sync_single_product_sku(
        ...
        try:
            ...
            logger.warning(f"⚠️ [NuvemshopProductService] Falha ao resolver categoria para SKU '{sku}': {cat_err}")
        ...
        if existing_nuvemshop_id:
            ...
            logger.info(f"🔄 [NuvemshopProductService] Produto SKU '{sku}' atualizado via PUT na Nuvemshop (ID: {nuvemshop_id}).")
        else:
            ...
            if ns_product and "id" in ns_product:
                ...
                logger.info(f"🔗 [NuvemshopProductService] SKU '{sku}' já existia na Nuvemshop (ID: {nuvemshop_id}). Atualizado via PUT.")
            else:
                ...
                logger.info(f"✨ [NuvemshopProductService] Produto SKU '{sku}' criado via POST na Nuvemshop (ID: {nuvemshop_id}).")
        ...
        if nuvemshop_id:
            try:
                ...
                logger.warning(
                    f"⚠️ [NuvemshopProductService] Falha isolada na sincronização de galeria para SKU '{sku}': {img_err}"
                )
