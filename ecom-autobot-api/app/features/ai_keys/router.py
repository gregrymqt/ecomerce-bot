...
async def test_ai_key(payload: AIKeyCreate):
    """
...
@router.post("/", status_code=status.HTTP_201_CREATED)
async def save_ai_key(payload: AIKeyCreate, tenant_id: str = "demo_tenant"):
    """
    Criptografa (AES-256 GCM) e salva a chave de API de um provedor de IA para o tenant.
    """
    try:
        await AIKeyService.save_key(tenant_id=tenant_id, payload=payload)
        return {
            "status": "success",
            "message": f"Chave de API do provedor '{payload.provider.value}' criptografada e salva com sucesso.",
        }
    except Exception as e:
        logger.error(f"Erro ao salvar chave de API para o tenant {tenant_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
