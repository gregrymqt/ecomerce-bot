# app/features/mercadopago/router.py
import logging
...
from app.features.mercadopago.schemas import MercadoPagoNotificationPayload, WebhookEventPayload
...
async def mercadopago_webhook(
    ...
    if not webhook_secret or not verify_mercadopago_signature(
        ...
        logger.warning("[Webhook] Assinatura HMAC inválida ou segredo ausente.")
        raise HTTPException(
    ...
    # 3. Publicação Rápida na fila de Webhooks (Sem recriar topologia)
    try:
        ...
        async with connection:
            ...
            await channel.default_exchange.publish(
                ...
                routing_key="webhook", # Apontando estritamente para a fila correta
            )
            logger.info(f"[Webhook] Notificação enfileirada com sucesso | Evento: {event_type}")
    except Exception as err:
        logger.error(f"[Webhook] Falha ao publicar evento no RabbitMQ: {err}")
