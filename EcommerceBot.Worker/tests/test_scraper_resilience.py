...
class TestScraperResilienceSuite(unittest.IsolatedAsyncioTestCase):
    ...
    # -------------------------------------------------------------------------
    # 2. Idempotência Operacional no Redis [QA-SCR-06]
    # -------------------------------------------------------------------------
    async def test_idempotency_prevents_duplicate_scraping(self):
        """Valida que mensagens duplicadas no RabbitMQ são descartadas sem gastar IA nem scrapers."""
        channel_mock = MagicMock()
        channel_mock.default_exchange.publish = AsyncMock()
        ...
        # Simula que a chave de idempotência já existe no Redis (processamento anterior nas últimas 24h)
        with patch("app.scraper.worker.redis_cache.is_already_processed", new=AsyncMock(return_value=True)):
            await _process_single_message(message_mock, channel_mock, parser_mock)
